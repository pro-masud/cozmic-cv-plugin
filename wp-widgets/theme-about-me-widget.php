...
if (class_exists('CSF')) {
    ...
    CSF::createWidget('cozmiq_about_me_widget', array(
        'title' => esc_html__('Cozmiq: About Me', 'cozmiq-cv'),
        'classname' => 'cozmiq-widget-about',
        'description' => esc_html__('Display about me widget', 'cozmiq-cv'),
        'fields' => array(
            array(
                'id' => 'heading',
                'type' => 'text',
                'title' => esc_html__('Enter Your Header Title', 'cozmiq-cv'),
                'default' => esc_html__('About Me', 'cozmiq-cv')
            ),
            array(
                'id' => 'trainer-area',
                'type' => 'media',
                'title' => esc_html__('Upload Your Trainer Photo', 'cozmiq-cv'),
            ),
            array(
                'id' => 'trainer_name',
                'type' => 'text',
                'title' => esc_html__('Enter Your Trainer Name', 'cozmiq-cv'),
                'default' => esc_html__('RANDALL SCHWARTZ', 'cozmiq-cv')
            ),
            array(
                'id' => 'description',
                'type' => 'textarea',
                'title' => esc_html__('Description', 'Cozmiq-cv'),
                'default' => esc_html__('Suspendisse vel nisl sed viverra tindunt. Vivamus et lobortis felis...', 'cozmiq-cv')
            ),
        )
    ));


    if (!function_exists('cozmiq_about_me_widget')) {
        function cozmiq_about_me_widget($args, $instance)
        {
