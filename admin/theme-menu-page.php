...
if (!class_exists('Cozmiq_CV_Admin_Menu')) {

    class Cozmiq_CV_Admin_Menu
    {
       /**
        * $instance
        * @since 1.0.0
        ...
        */
        ...
        public function theme_admin_menu_page()
        {
            ...
            //add menu page
            add_menu_page(
                esc_html__('Cozmiq Page', 'cozmiq-cv'),
                esc_html__('Cozmiq', 'cozmiq-cv'),
                'manage_options',
                'cozmiq_theme_options',
                array($this, 'admin_options_fallback_function'),
                COZMIQ_CV_ADMIN_ASSETS . '/img/icon.png',
                80
            );
        }
            ...
            $tab_menus = array(
                'event' => array(
                    array(
                        ...
                        'name' => sprintf(esc_html__('%s', 'cozmiq-cv'), 'Event'),
                        'id' => 'edit-event'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=event-cat&post_type=event',
                        'name' => sprintf(esc_html__('%s Categories', 'cozmiq-cv'), 'Event'),
                        'id' => 'edit-event-cat'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=event-tag&post_type=event',
                        'name' => sprintf(esc_html__('%s Tag', 'cozmiq-cv'), 'Event'),
                        'id' => 'edit-event-tag'
                    )
                ),
                'project' => array(
                    array(
                        ...
                        'name' => sprintf(esc_html__('%s', 'cozmiq-cv'), 'Project'),
                        'id' => 'edit-project'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=project-cat&post_type=project',
                        'name' => sprintf(esc_html__('%s Categories', 'cozmiq-cv'), 'Project'),
                        'id' => 'edit-project-cat'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=project-tag&post_type=project',
                        'name' => sprintf(esc_html__('%s Tag', 'cozmiq-cv'), 'Project'),
                        'id' => 'edit-project-tag'
                    )
                ),
                'team' => array(
                    array(
                        ...
                        'name' => sprintf(esc_html__('%s', 'cozmiq-cv'), 'Team'),
                        'id' => 'edit-team'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=team-cat&post_type=team',
                        'name' => sprintf(esc_html__('%s Categories', 'cozmiq-cv'), 'Team'),
                        'id' => 'edit-team-cat'
                    ),
                    array(
                        'link' => 'edit-tags.php?taxonomy=team-tag&post_type=team',
                        'name' => sprintf(esc_html__('%s Tag', 'cozmiq-cv'), 'Team'),
                        'id' => 'edit-team-tag'
                    )
                )
            );
        ...
        public function admin_menu_activation()
        {
            ...
            <script type="text/javascript">
                (function ($) {
                    ...
                    mainwrap = $('#toplevel_page_cozmiq_theme_options');
                    ...
                    if (check) {
                        $('.wp-submenu.wp-submenu-wrap')
                            .find('a[href*="admin.php?page=toplevel_page_cozmiq_theme_options"]')
                            .removeClass('current')
                            .parent('li')
                            .removeClass('current');
                    }
    ...
    if (class_exists('Cozmiq_CV_Admin_Menu')) {
        Cozmiq_CV_Admin_Menu::getInstance();
    }
