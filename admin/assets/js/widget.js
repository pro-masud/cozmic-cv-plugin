(function ($) {
  ...
  $(document).ready(function () {
    /***********************************
         cozmiq Custom widget js
         ************************************/
    $(document).on("widget-updated", function (e, widget) {
      var widget_id = $(widget).attr("id");
      if (widget_id.indexOf("cozmiq_about_us_widget") != 1) {
        prefetch();
      }
    });

    $("body").off("click", ".cozmiq_flogo_uploader");

    $("body").on("click", ".cozmiq_flogo_uploader", function (e) {
      var el = this;
      ...
      file_uploder_frame.on("insert", function () {
        ...
        var img_prev_container = $(el).siblings(".cozmiq-logo-preview");
        ...
        $(el)
          .siblings()
          .children(".cozmiq_logo_id")
          .val(selected_ids.join(","));
        $(el).siblings().children(".cozmiq_logo_id").trigger("change");
      ...
      file_uploder_frame.on("open", function () {
        ...
        console.log($(el).siblings().children(".cozmiq_logo_id").val());
        var atts = $(el)
          .siblings()
          .children(".cozmiq_logo_id")
          .val()
    ...
    function prefetch() {
      $(".cozmiq_logo_id").each(function () {
        var attid = $(this).val();
        var container = $(this).parent().siblings(".cozmiq-logo-preview");
        container.html("");
        ...
        if (attid) {
          $(this)
            .parent()
            .parent()
            .find(".cozmiq_flogo_uploader")
            .val("Change Image");
          var attachment = new wp.media.model.Attachment.get(attid);
          attachment.fetch({
            success: function (att) {
              container.append(
                '<img src="' + att.attributes.sizes.full.url + '"/>'
              );
            },
          });
        }
