...
if (!class_exists('Cozmiq_Post_Column_Customize')){
    class Cozmiq_Post_Column_Customize{
        //$instance variable
        ...
        public function __construct() {
            ...
            add_action('manage_project_posts_custom_column', array($this, 'add_project_thumbnail_columns'), 20,4);
        ...
        public function edit_event_columns($columns){
            ...
            $columns['title'] = esc_html__('Title','cozmiq-cv');
            $columns['thumbnail'] = '<a href="edit.php?post_type=event&orderby=title&order='.urlencode($order).'">'.esc_html__('Thumbnail','cozmiq-cv').'</a>';
            ...
            $columns['icon'] = esc_html__('Icon','cozmiq-cv');
            $columns['date'] = esc_html__('Date','cozmiq-cv');
            return $columns;
        }
        ...
        public function add_event_thumbnail_columns($column,$post_id) {
            switch ( $column ) {
                case 'thumbnail' :
                    ...
                    break;
                case 'icon' :
                    $event_meta_option = get_post_meta($post_id ,'cozmiq_event_options', true);
        ...
        public function add_event_category_columns($string,$columns,$post_id){
            $post_term_meta = get_term_meta($post_id,'cozmiq_event_category',true);
        ...
        public function edit_project_columns($columns){
            ...
            $columns['cb'] = '<input type="checkbox" />';
            $columns['title'] = esc_html__('Title','cozmiq-cv');
            $columns['thumbnail'] = '<a href="edit.php?post_type=project&orderby=title&order='.urlencode($order).'">'.esc_html__('Thumbnail','cozmiq-cv').'</a>';
        ...
        public function add_project_thumbnail_columns($column,$post_id) {
            switch ( $column ) {
                case 'thumbnail' :
                    ...
                    break;
                // case 'icon' :
                //     $project_meta_option = get_post_meta($post_id ,'cozmiq_project_options', true);
        ...
        public function add_project_category_columns($string,$columns,$post_id){
            $post_term_meta = get_term_meta($post_id,'cozmiq_project_category',true);
            $icon = isset($post_term_meta['icon']) ? $post_term_meta['icon'] : '';
            switch ( $columns ) {
                case 'icon' :
                    echo '<i class="neaterller-font-size50 '.$icon.'"></i>';
        ...
        public function edit_team_columns($columns){
            ...
            $columns['cb'] = '<input type="checkbox" />';
            $columns['title'] = esc_html__('Title','cozmiq-cv');
            $columns['thumbnail'] = '<a href="edit.php?post_type=team&orderby=title&order='.urlencode($order).'">'.esc_html__('Thumbnail','cozmiq-cv').'</a>';
        ...
        public function add_team_thumbnail_columns($column,$post_id) {
            switch ( $column ) {
                case 'thumbnail' :
                    ...
                    break;
                case 'icon' :
                    $team_meta_option = get_post_meta($post_id ,'cozmiq_team_options', true);
        ...
        public function add_team_category_columns($string,$columns,$post_id){
            $post_term_meta = get_term_meta($post_id,'cozmiq_team_category',true);
            $icon = isset($post_term_meta['icon']) ? $post_term_meta['icon'] : '';
            switch ( $columns ) {
                case 'icon' :
                    echo '<i class="neaterller-font-size50 '.$icon.'"></i>';
    ...
    if ( class_exists('Cozmiq_Post_Column_Customize')){
        Cozmiq_Post_Column_Customize::getInstance();
    }
}