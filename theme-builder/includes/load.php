...
use \cozmiqhead_\Dtdr_Controller as Contr;
...
class Dtdr_Load{
    ...
    public $option_keys = 'hugebinary-dark-options-settings';
    ...
    public function init_menu(){
        ...
        do_action('cozmiq_header_admin_menu_start');
        
        add_menu_page(
			__( 'Create Template', 'cozmiqhead_' ),
			__( 'Header & Footer Builder', 'cozmiqhead_' ),
			'edit_pages',
            'edit.php?post_type=hugebinary-builder',
            '',
            'dashicons-chart-pie',
            7
        );

        do_action('cozmiq_header_admin_menu_end');
    }
    
    public function hidden_admin_notices(){
        $screen = get_current_screen();
        if( in_array($screen->id, [ 'toplevel_page_hugebinary-head-settings']) ){
    ...
    public function admin_enqueue(){
        ...
        if( in_array($screen->id, [ 'toplevel_page_hugebinary-head-settings', 'hugebinary-builder', 'edit-hugebinary-builder']) ){
        ...
        if( ( 'post.php' == $pagenow || 'post-new.php' == $pagenow ) || in_array($screen->id, [ 'toplevel_page_hugebinary-head-settings', 'hugebinary-builder', 'edit-hugebinary-builder']) ){
