...
class Common{
    ...
    public function template_type_array( $index = ''){
        $type = apply_filters('cozmiq_head_template_type', [
            'header' => __('Header', 'cozmiq-cv'),
            'footer' => __('Footer', 'cozmiq-cv'),
            'mega_menu' => __('Mega Menu', 'cozmiq-cv'),
            'page-banner' => __('Page Banner', 'cozmiq-cv'),
            'f0f' => __('404', 'cozmiq-cv'),
        ]);
        if( !empty($index) ){
            return ($type[$index]) ?? '';
        }
    ...
    public function working_posttype(){
        ...
        return apply_filters('cozmiq_head_template_select_posttype', array_values($post_types) );
    }
