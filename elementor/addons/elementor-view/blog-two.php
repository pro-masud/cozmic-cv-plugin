        ...
        <div class="container">
            <div class="gt-section-title-area">
                ...
                <a href="<?php echo esc_url($settings['layout_two_button_url']['url']); ?>" class="gt-theme-btn wow fadeInUp" data-wow-delay=".4s">
                    <?php echo wp_kses( $settings['layout_two_btn_label'], 'cozmiq_allowed_tags' ); ?>
                </a>
            </div>
            <div class="row">
                <?php
                    $blog_post_one_query_paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
                    if (! empty($settings['layout_two_select_category'])) :
                        $blog_post_one_query_args = array(
                            'post_type' => 'post',
                            'post_status' => 'publish',
                            'ignore_sticky_posts' => true,
                            'orderby' => 'date',
                            'order' => $settings['layout_two_query_order'],
                            'posts_per_page' => $settings['layout_two_post_count']['size'],
                            'paged' => $blog_post_one_query_paged,
                            'tax_query' => array(
                                array(
                                    'taxonomy' => 'category',
                                    'field' => 'slug',
                                    'terms' => $settings['layout_two_select_category']
                                )
                            )
                        );
                    else :
                            ...
                            <div class="col-lg-6 wow fadeInUp" data-wow-delay=".3s">
                                <div class="gt-news-box-item-2">
                                    ...
                                    <div class="gt-content">
                                        <ul class="gt-list">
                                            <li>
                                                <?php cozmiq_elementor_rendered_image($settings, 'layout_two_date_shape' ); ?>
